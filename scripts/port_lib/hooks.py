...
Per-target event support:
    ...
    Claude:    9 events (PreToolUse, PostToolUse, PostToolUseFailure,
               ...
               StopFailure, Stop)
    Codex:     7 of 9 — drops PostToolUseFailure, StopFailure.
               SubagentStart is supported since codex-cli 0.133.0
               (openai/codex PR #22782) — Iron Laws injection works.
...
CODEX_SUPPORTED_EVENTS = {
    ...
    "PreCompact",
    "PostCompact",
    # Since codex-cli 0.133.0 (PR #22782): SubagentStart with
    # additionalContext injection — same contract as Claude Code.
    "SubagentStart",
}

# Source-only events (no Codex equivalent). Documented in `docs/multi-agent/codex.md`.
CODEX_DROPPED_EVENTS = {"PostToolUseFailure", "StopFailure"}
...
def render_codex_hooks(
    ...
    Rewrites `${CLAUDE_PLUGIN_ROOT}` to `${PLUGIN_ROOT}` in script paths
    inside `hooks.json` — verified against codex-rs/hooks/src/engine/
    discovery.rs: Codex injects `PLUGIN_ROOT` (native) and
    `CLAUDE_PLUGIN_ROOT` (compat) into plugin hook env; the previously
    used `CODEX_PLUGIN_ROOT` does NOT exist and broke every hook path.
    Only ships shell scripts that the resulting
    ...
    for event, configs in (source.get("hooks") or {}).items():
        ...
        rewritten = json.loads(
            json.dumps(configs).replace("${CLAUDE_PLUGIN_ROOT}", "${PLUGIN_ROOT}")
        )
    ...
    install_entry = {
        ...
        "hooks": [
            {
                "type": "command",
                "command": "${PLUGIN_ROOT}/hooks/scripts/install-codex-agents.sh",
                "timeout": 30,
    ...
    # PostToolUseFailure / StopFailure scripts) are skipped.
 ...
 *
 * API verified against sst/opencode packages/plugin/src/index.ts (v1.17.x):
 * - module shape is a default-export PluginModule { id?, server: Plugin };
 *   `Plugin` is a FUNCTION (input) => Promise<Hooks>, not a hooks object
 * - every hook receives (input, output) as TWO parameters
 * - `experimental.chat.system.transform` mutates output.system (a string[])
 * - `event` receives { event: Event } where Event is an object with .type
 */
...
const server: Plugin = async (_input) => {
  const hooks: Hooks = {
    // PreToolUse — block dangerous ops (mix ecto.reset, git push --force,
    // MIX_ENV=prod). input carries { tool, sessionID, callID }; the tool
    // arguments live on output.args. Throwing aborts the tool call.
    ...
    // input carries { tool, sessionID, callID, args }. Fire-and-forget so the
    // editor doesn't block on three subprocesses per .ex/.exs save.
    "tool.execute.after": async (input, _output) => {
      if (input.tool !== "edit" && input.tool !== "write") return;
      const args = input.args ?? {};
      const path = (args.filePath ?? args.file_path ?? args.path ?? "") as string;
      if (!path.endsWith(".ex") && !path.endsWith(".exs")) return;
      const { spawn } = await import("child_process");
      // server.ts sits at the plugin root; resolve scripts relative to it so
      // the plugin works from any install location (file:// or npm).
      ...
      // (`.tool_input.file_path`), so feed them that exact shape. Pipe stdin,
      // ignore output, unref so the editor never blocks on the subprocesses.
    ...
    // Inject Iron Laws into the system prompt (equivalent of Claude's
    // SubagentStart injection). output.system is a string[] — push, don't return.
    "experimental.chat.system.transform": async (_input, output) => {
      output.system.push(IRON_LAWS);
    },

    // PreCompact-equivalent: carry the Iron Laws through session compaction
    // so they survive into the compacted context.
    "experimental.session.compacting": async (_input, output) => {
      output.context.push(IRON_LAWS);
    },

    // event filter — SessionStart-equivalent. Currently a no-op; placeholder
    // for future workflow-state restoration. input.event is an object.
    event: async (input) => {
 ...
 * API re-verified against the 0.79.1 type declarations):
 * - ToolCallEvent carries `toolName` / `input` (NOT `tool` / `args`)
 * - blocking a tool call is the typed return { block: true, reason } —
 *   throwing is undefined behaviour, not the documented block contract
 */
...
export default function (pi: ExtensionAPI) {
  ...
  pi.on("tool_call", (event: any) => {
    ...
    if (/mix\\s+ecto\\.(reset|drop)/.test(cmd)) {
      return {
        block: true,
        reason: "BLOCKED: destructive ecto operation. Use a migration instead.",
      };
    }
    if (/git\\s+push\\b(?=[^\\n]*--force)(?![^\\n]*--force-with-lease)/.test(cmd)) {
      return {
        block: true,
        reason: "BLOCKED: --force push. Use --force-with-lease at minimum.",
      };
    }
  });
}
 ...
 * (>=0.74.0; API re-verified against the 0.79.1 type declarations).
 *
 * sendUserMessage lives on the top-level ExtensionAPI (`pi`), NOT on the
 * command handler's ExtensionCommandContext — calling ctx.sendUserMessage
 * is a runtime TypeError. The factory captures `pi` in closure instead.
 */
  ...
  for (const name of COMMANDS) {
    pi.registerCommand(name, {
      ...
      handler: async (args: string, _ctx: any) => {
        pi.sendUserMessage(`/${name} ${args}`.trim());
      },
    });
  }
}
