...
PI_PACKAGE_NAME = "pi-elixir-phoenix"
...
def _package_manifest(source_plugin_dir: str | Path) -> dict:
    ...
    return {
        # Pi package identity is public and must not follow Claude's runtime
        # namespace, which is intentionally `phx` for legacy slash commands.
...
def validate(output_dir: str | Path, expected_manifest: dict | None = None) -> int:
    ...
    try:
        ...
        raise ValueError(
            f"{manifest_file}: invalid or missing Pi package manifest"
        ) from error
    ...
    if pi_manifest != {"skills": ["./skills"]}:
        raise ValueError(
            f"{manifest_file}: Pi baseline must declare only a skills array"
        )
    ...
    for skill_file in skill_files:
        ...
        if (
            ...
            raise ValueError(
                f"{skill_file}: invalid or mismatched Pi skill name `{name}`"
            )
