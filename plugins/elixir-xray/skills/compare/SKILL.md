...
# X-Ray Compare — Scan Diff

Compare the current scan against a previous scan to track improvement over time.

```
/xray:compare                          # Auto-find most recent history scan
/xray:compare .claude/xray/history/scan-2026-03-15.json  # Specific baseline
```

## Prerequisites

- `.claude/xray/findings-merged.json` from a recent `/xray:scan`
- At least one previous scan in `.claude/xray/history/scan-*.json`
  (created automatically by `/xray:scan` — see scan skill's history saving)
...
Read `.claude/xray/findings-merged.json`.
If not found: "No current scan results. Run `/xray:scan` first."
...
Otherwise, use Glob to find `.claude/xray/history/scan-*.json` files.
Pick the most recent one (by filename date sort). If none found:
"No scan history found. Run `/xray:scan` again after your changes — the scan auto-saves history."
