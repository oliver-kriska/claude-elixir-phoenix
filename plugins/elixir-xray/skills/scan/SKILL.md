...
```
/xray:scan              # Standard 6-layer analysis (~50 findings, 10 min)
/xray:scan --quick      # Quick: Layers 1, 3, 6 only (~3 min)
/xray:scan --full       # Standard + deep domain analysis on Layer 3
/xray:scan --deep       # Deep: 20 sub-agents, 100+ findings, 60-100KB detailed report (~20 min)
/xray:scan --pr 42      # PR-scoped: only changed files, 3 layers (~2 min)
/xray:scan --gate measure  # Create quality baseline for ratcheting
/xray:scan --gate check    # Check current state against baseline (CI integration)
/xray:scan --focus=prs  # Single layer (git|prs|code|config|sessions|arch)
/xray:scan --since 2025-10-01
```
...
4. **NEVER write generated artifacts** — scan is read-only analysis; use `/xray:apply` for artifact generation
...
Tell user: "Starting X-Ray scan{mode_label}. You'll need to approve one Bash command."
...
**Script cache**: Before running scripts, check if `.claude/xray/layers/*.json` files
...
```bash
mkdir -p .claude/xray/layers/sessions .claude/xray/layers/{L1,L2,L3,L4,L5,L6} && S="{SCRIPTS}" && P="{PROJECT_ROOT}" && python3 "$S/analyze-git-history.py" "$P" --since "{SINCE}" > .claude/xray/layers/git-history.json 2>.claude/xray/layers/git-history.err & python3 "$S/analyze-prs.py" "$P" --since "{SINCE}" > .claude/xray/layers/pr-reviews.json 2>.claude/xray/layers/pr-reviews.err & python3 "$S/analyze-code.py" "$P" --full > .claude/xray/layers/code-docs.json 2>.claude/xray/layers/code-docs.err & bash "$S/analyze-config.sh" "$P" > .claude/xray/layers/claude-config.json 2>.claude/xray/layers/claude-config.err & bash "$S/analyze-architecture.sh" "$P" > .claude/xray/layers/architecture.json 2>.claude/xray/layers/architecture.err & python3 "$S/temporal-coupling.py" "$P" --since "{SINCE}" > .claude/xray/layers/temporal-coupling.json 2>.claude/xray/layers/temporal-coupling.err & python3 "$S/hotspot-score.py" "$P" --since "{SINCE}" > .claude/xray/layers/hotspot-score.json 2>.claude/xray/layers/hotspot-score.err & wait && echo "Done" && wc -c .claude/xray/layers/*.json
```
...
```bash
S="{SCRIPTS}" && for f in .claude/xray/layers/sessions/_tmp_*.json; do ID=$(basename "$f" .json | sed 's/_tmp_//'); python3 "$S/analyze-sessions.py" "$f" --session-id "$ID" > ".claude/xray/layers/sessions/session-${ID}.json" && rm "$f"; done && python3 "$S/analyze-sessions.py" .claude/xray/layers/sessions/ --mode aggregate > .claude/xray/layers/sessions-summary.json
```
...
```
Agent(subagent_type="elixir-xray:L1-orchestrator", prompt="Deep Layer 1 analysis.\nGIT_HISTORY={PROJECT_ROOT}/.claude/xray/layers/git-history.json\nTEMPORAL_COUPLING={PROJECT_ROOT}/.claude/xray/layers/temporal-coupling.json\nHOTSPOT_SCORE={PROJECT_ROOT}/.claude/xray/layers/hotspot-score.json\nOUTPUT_DIR={PROJECT_ROOT}/.claude/xray/layers/L1\nWrite consolidated findings to OUTPUT_DIR/consolidated.md", run_in_background=true)

Agent(subagent_type="elixir-xray:L2-orchestrator", prompt="Deep Layer 2 analysis.\nPR_REVIEWS={PROJECT_ROOT}/.claude/xray/layers/pr-reviews.json\nOUTPUT_DIR={PROJECT_ROOT}/.claude/xray/layers/L2\nWrite consolidated findings to OUTPUT_DIR/consolidated.md", run_in_background=true)

Agent(subagent_type="elixir-xray:L3-orchestrator", prompt="Deep Layer 3 analysis.\nCODE_DOCS={PROJECT_ROOT}/.claude/xray/layers/code-docs.json\nOUTPUT_DIR={PROJECT_ROOT}/.claude/xray/layers/L3\nWrite consolidated findings to OUTPUT_DIR/consolidated.md", run_in_background=true)

Agent(subagent_type="elixir-xray:L4-orchestrator", prompt="Deep Layer 4 analysis.\nCLAUDE_CONFIG={PROJECT_ROOT}/.claude/xray/layers/claude-config.json\nCODE_DOCS={PROJECT_ROOT}/.claude/xray/layers/code-docs.json\nOUTPUT_DIR={PROJECT_ROOT}/.claude/xray/layers/L4\nWrite consolidated findings to OUTPUT_DIR/consolidated.md", run_in_background=true)

Agent(subagent_type="elixir-xray:L5-orchestrator", prompt="Deep Layer 5 analysis.\nSESSIONS={PROJECT_ROOT}/.claude/xray/layers/sessions-summary.json\nOUTPUT_DIR={PROJECT_ROOT}/.claude/xray/layers/L5\nWrite consolidated findings to OUTPUT_DIR/consolidated.md", run_in_background=true)

Agent(subagent_type="elixir-xray:L6-orchestrator", prompt="Deep Layer 6 analysis.\nARCHITECTURE={PROJECT_ROOT}/.claude/xray/layers/architecture.json\nOUTPUT_DIR={PROJECT_ROOT}/.claude/xray/layers/L6\nWrite consolidated findings to OUTPUT_DIR/consolidated.md", run_in_background=true)
```
...
```bash
python3 "{SCRIPTS}/merge-findings.py" .claude/xray/layers/ > .claude/xray/findings-merged.json
```
...
Write to `.claude/xray/`. Present dashboard + summary to user.

**History saving** (enables `/xray:compare`): After writing reports, save a timestamped copy:

```bash
mkdir -p .claude/xray/history/ && cp .claude/xray/findings-merged.json .claude/xray/history/scan-$(date +%Y-%m-%d).json
```
...
- `${CLAUDE_SKILL_DIR}/references/history-saving.md` — Scan history for `/xray:compare`
