...
name: xray:intro
description: >
  Walk through the Elixir X-Ray plugin commands and Credo check generation workflow.
  Use when a new user asks what is X-Ray, how does xray:scan work, what commands are
  available, or needs an introduction to codebase audit and review features.
effort: low
argument-hint: ""
---

# Elixir X-Ray — Introduction

Welcome to the Elixir X-Ray plugin! This plugin analyzes your Elixir/Phoenix codebase across **6 layers** and generates actionable improvements.

## What It Does

X-Ray scans your project's **code, git history, pull requests, documentation,
...
```
/xray:scan              # Full 6-layer analysis
/xray:scan --quick      # Quick scan (3 layers: git, code, architecture)
/xray:scan --full       # Deep domain analysis mode
/xray:apply             # Generate all artifacts from scan results
/xray:apply --pick      # Cherry-pick which artifacts to generate
/xray:brief             # Interactive walkthrough of findings
```

## Typical Workflow

```
1. /xray:scan           → Analyzes your project (~5-10 min full, ~2-3 min quick)
2. /xray:brief          → Walk through findings, understand what was found
3. /xray:apply --pick   → Generate the artifacts you want
4. Review & adopt     → Copy generated files from .claude/xray/generated/
```

## Example Output

After running `/xray:scan`, you'll see a dashboard like:
...
Ready to start? Run `/xray:scan` on your project!
