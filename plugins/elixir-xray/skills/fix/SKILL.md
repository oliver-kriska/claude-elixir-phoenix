...
# X-Ray Fix — Apply Highest-ROI Finding

Generate AND apply fixes directly to your project from scan findings.

Unlike `/xray:apply` (which stages artifacts in `.claude/xray/generated/`),
`/xray:fix` writes directly to project files — Credo checks into `lib/`,
CI steps into workflows, rules into `CLAUDE.md`.

## Usage

```
/xray:fix              # Interactive menu — pick findings to fix
/xray:fix L3-001       # Fix a specific finding by ID
/xray:fix --top 3      # Fix the top 3 by ROI score
/xray:fix --all        # Fix all automatable findings (batch)
```
...
```
X-Ray found {N} automatable findings. What would you like to fix?
...
3. Append under an `## X-Ray Rules` section (create section if missing)
...
3. If still failing: revert the file, report failure, suggest `/xray:apply` for manual review
