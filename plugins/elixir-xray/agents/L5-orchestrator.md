...
description: |
  ...
  debugging pattern analyzer. Used by /xray:scan --deep.
...
- **Primary**: `.claude/xray/layers/sessions-summary.json`
...
- `.claude/xray/layers/L5/recurring-tasks.md`
- `.claude/xray/layers/L5/debugging-patterns.md`
...
Write `.claude/xray/layers/L5/consolidated.md` with:
...
- Sub-agent fails → log to `.claude/xray/layers/L5/errors.log`, continue with remaining
...
```
You are a specialist analyzer for Elixir X-Ray Layer 5.
