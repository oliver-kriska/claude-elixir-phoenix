...
| `/xray:scan --quick` | Quick 3-layer scan (git, code, architecture, ~3 min) |
| `/xray:scan --full` | Standard + deep domain analysis on Layer 3 |
| `/xray:scan --deep` | Deep: 20 sub-agents, 100+ findings, detailed report (~20 min) |
| `/xray:scan --gate measure` | Create quality baseline for CI ratcheting |
| `/xray:scan --gate check` | Check current state against baseline (CI integration) |
| `/xray:apply` | Generate artifacts from scan results |
| `/xray:apply --pick` | Interactive cherry-pick which artifacts to generate |
| `/xray:brief` | Interactive walkthrough of scan findings |
| `/xray:compare` | Compare current scan against previous to show progress/regressions |
| `/xray:intro` | Plugin introduction and usage guide |
...
├── history/                 # Timestamped scan snapshots for /xray:compare
│   ├── scan-2026-03-01.json
│   └── scan-2026-03-15.json
├── generated/               # Artifacts from /xray:apply
...
5. **Generated artifacts stay in .claude/xray/generated/** — never write to project lib/ directly
...
```json
{
  "permissions": {
    "allow": [
      "Bash(python3 **/elixir-xray/**)",
      "Bash(bash **/elixir-xray/**)",
      "Bash(SCRIPTS=*)",
      "Bash(mkdir -p *)"
    ]
  }
