...
```
Agent(
  subagent_type="elixir-inspector:{agent-name}",
  prompt="Analyze the pre-computed data at {JSON_PATH}. Write findings to .claude/inspector/layers/{layer}.md using the finding schema (YAML frontmatter with id, layer, category, title, severity, effort, automatable, artifact_types, evidence, frequency, confidence). Report ONLY issues found.",
...
Agent(
  ...
  prompt="""
Compress Inspector layer findings for synthesis.
Input directory: .claude/inspector/layers/
Files to read: git-history.md, pr-reviews.md, code-docs.md, claude-config.md, sessions.md, architecture.md
Output: .claude/inspector/layers/summary.md
...
Inspector agents don't have MCP access.
