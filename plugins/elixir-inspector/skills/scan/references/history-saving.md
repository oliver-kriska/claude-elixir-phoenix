...
a timestamped copy for future comparison via `/ei:compare`.
...
1. Create the history directory:
   ...
   ```bash
   mkdir -p .claude/inspector/history/
   ```

2. Copy the merged findings with a date stamp:

   ```bash
   cp .claude/inspector/findings-merged.json \
      .claude/inspector/history/scan-$(date +%Y-%m-%d).json
   ```
...
- `/ei:compare` reads these files by glob-sorting on filename date
