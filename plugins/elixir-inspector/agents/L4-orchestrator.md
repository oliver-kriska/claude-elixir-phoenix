...
description: |
  ...
  missing rule detector. Used by /ei:scan --deep.
...
- **Primary**: `.claude/inspector/layers/claude-config.json`
- **Cross-reference**: `.claude/inspector/layers/code-docs.json`
...
```bash
mkdir -p .claude/inspector/layers/L4
```
...
- `.claude/inspector/layers/L4/rule-enforcement.md`
- `.claude/inspector/layers/L4/missing-rules.md`
...
Write `.claude/inspector/layers/L4/consolidated.md` with:
...
- Sub-agent fails → log to `.claude/inspector/layers/L4/errors.log`, continue with remaining
...
```
You are a specialist analyzer for Elixir Inspector Layer 4.
