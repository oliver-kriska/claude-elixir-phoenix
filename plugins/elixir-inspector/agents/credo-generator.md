...
description: |
  Generate custom Credo checks from Inspector findings.
...
Generate custom Credo checks from Inspector scan findings.
...
3. Write to `.claude/inspector/generated/credo-checks/{check_name}.ex`
...
1. One `.ex` file per Credo check in `.claude/inspector/generated/credo-checks/`
...
After generating all checks, report which ones were created. The `/ei:apply` orchestrator will run `mix compile` to validate.
