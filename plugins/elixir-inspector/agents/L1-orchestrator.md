...
description: |
  Deep Layer 1 orchestrator for Inspector. Spawns 4 specialist sub-agents to analyze
  git history patterns. Used by /ei:scan --deep. Returns consolidated findings.
...
1. Write the raw output to `.claude/inspector/layers/L1/{name}.md`:
   - L1a → `fix-categorizer.md`
   - L1b → `co-change.md`
   - L1c → `developer-patterns.md`
   - L1d → `hotspot-trends.md`

2. For failed sub-agents, write to `.claude/inspector/layers/L1/errors.log`:
...
Write `.claude/inspector/layers/L1/consolidated.md`:
...
1. Log the error to `.claude/inspector/layers/L1/errors.log`
...
Agent({
  ...
  prompt: "Analyze git history. Input: {GIT_HISTORY_JSON_PATH}. Write findings to .claude/inspector/layers/L1/consolidated.md",
...
```bash
mkdir -p .claude/inspector/layers/L1
```

## Output

Your final output is `.claude/inspector/layers/L1/consolidated.md`. The scan
