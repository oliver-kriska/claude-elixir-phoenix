...
All notable changes to the Elixir Inspector plugin.

## [0.9.0-beta] - 2026-03-23

### Added

- `/ei:compare` skill — compare scans to show resolved findings, new issues, severity changes
- `/ei:fix` skill — interactive menu to auto-implement findings (pick all, top N, category, or specific IDs)
- `/ei:scan --pr NUMBER` — PR-scoped scanning, 3 layers, ~2 min
...
- Scan history saving for `/ei:compare`

### Changed

- `/ei:fix` now interactive by default (grouped menu with ROI scores)
...
- **Deep scan mode** (`/ei:scan --deep`) — 6 orchestrator agents spawn 20 specialist sub-agents
- **Quality gate** (`/ei:scan --gate measure/check`) — baseline ratcheting for CI integration
...
- 4 skills (/ei:scan, /ei:apply, /ei:brief, /ei:intro)
