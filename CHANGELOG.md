...
- **Policy parser was taking the FIRST regex match** in
  `hex_vet.exs`, so any earlier substring matching
  `block_on_unvetted: <atom>` won — including commented examples like
  `# block_on_unvetted: false  # see migration guide`. Result: gates
  configured for `:strict` or `:new_only` silently ran in warn-only
  mode whenever the ledger contained a documentation-friendly comment.
  Fail-open in the worst possible direction.
- **Fix**: strip Elixir line-comments (`sed 's/#.*$//'`) before
  matching, take the LAST uncommented match (matches Elixir
  map-literal last-assignment-wins semantics). Defense-in-depth: emit
  a stderr warning when more than one uncommented key is found rather
  than silently picking one.
- **Verify your effective policy** with:
