...
description: Deep bug investigation using 4 parallel subagents (reproduction, root
  cause, impact, fix strategy). Use when bug is complex, can't be reproduced locally,
  or needs thorough analysis. Spawns fresh-context subagents for each investigation
  track.
model: sonnet
mode: subagent
skills:
