...
("`Run /skill:phx-deps-audit for full pipeline`") and exits 0.
...
findings, and points the user at `/skill:phx-deps-audit` for the full Tier 2
pipeline. Reason: hook-budget exhaustion via the Bash 600s timeout is
worse UX than an explicit "run the audit" message.

The `/skill:phx-deps-audit` skill body owns Tier 2 — when invoked manually,
...
```json
{
  "type": "command",
  "if": "Bash(*mix deps.*)",
  "command": "[Claude Code-only hook unavailable in the Pi skills-only package: hooks/scripts/deps-audit-gate.sh]",
  "timeout": 5,
