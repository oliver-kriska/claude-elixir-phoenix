...
Phase 3 adds non-interactive `--ci` mode to `/skill:phx-deps-audit` for use
...
The `--ci` flag works with `/skill:phx-deps-audit` inside Claude Code AND
...
```bash
claude code -m "/skill:phx-deps-audit --ci --base origin/main --sarif audit.sarif"
```
