...
```
/skill:phx-deps-audit --base main
/skill:phx-deps-audit --base origin/main
/skill:phx-deps-audit --base abc1234
```
...
| **Cost** | Same as Mode B |
| **CI hint** | `/skill:phx-deps-audit --base origin/main --json` for machine-readable output |
...
```
/skill:phx-deps-audit --preview                 # all locked deps vs latest
/skill:phx-deps-audit --preview httpoison       # one package
/skill:phx-deps-audit --preview httpoison req   # multiple
```
...
| Situation | Command |
|-----------|---------|
| Just ran `mix deps.update`, want pre-commit check | `/skill:phx-deps-audit` |
| Reviewing a PR that bumped `mix.lock` | `/skill:phx-deps-audit --base origin/main` |
| Considering whether to update `httpoison` | `/skill:phx-deps-audit --preview httpoison` |
| Curious which deps could update | `/skill:phx-deps-audit --preview` (capped at 50) |
| Just merged main, want fresh audit on whole tree | `git fetch && /skill:phx-deps-audit --base origin/main~1` |
