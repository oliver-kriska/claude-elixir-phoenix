...
Task creation and planning happen in `/skill:phx-plan` after the user decides.
...
Ready for: /skill:phx-learn-from-fix (capture lessons)
```
...
- `/skill:phx-plan` — Plan tests for these functions
- `/skill:phx-work` — Write tests directly
...
- `/skill:phx-plan .claude/plans/{slug}/reviews/{file}.md` — Replan fixes
  (best for complex or architectural issues)
- `/skill:phx-work` — Fix directly
