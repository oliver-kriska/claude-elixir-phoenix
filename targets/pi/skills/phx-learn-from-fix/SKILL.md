...
```
/skill:phx-learn-from-fix Fixed N+1 query in user listing - was missing preload
/skill:phx-learn-from-fix String vs atom key mismatch in params handling
/skill:phx-learn-from-fix LiveView assign_async needs render_async in tests
/skill:phx-learn-from-fix --library ical --scope personal ICal.to_ics output needs CRLF line endings
/skill:phx-learn-from-fix --library ical --scope project Use method: "PUBLISH" for calendar feeds
```
...
unsolved error. Use `/skill:phx-compound` for a detailed completed investigation.
...
| Detailed completed fix | `.claude/solutions/` via `/skill:phx-compound` | Debugging narrative |
