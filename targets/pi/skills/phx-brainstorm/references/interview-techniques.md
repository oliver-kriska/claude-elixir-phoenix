...
Detailed methodology for the adaptive interview phase of `/skill:phx-brainstorm`.
...
- **Summary section** is what `/skill:phx-plan` reads first — must be self-contained
- **Coverage Details** replace `/skill:phx-plan`'s clarification questions
- **Codebase Context** replaces the patterns-analyst research in `/skill:phx-plan`
- **Transcript** at the bottom for audit trail — not consumed by `/skill:phx-plan`
