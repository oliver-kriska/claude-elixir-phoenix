...
Detailed step-by-step execution for `/skill:phx-full`.
...
Run `/skill:phx-plan {feature}` (with `--depth deep` for "research it"):
...
Run `/skill:phx-work .claude/plans/{feature}/plan.md`:
...
Run `/skill:phx-review` (append `--codex` when the full run was invoked with
`--codex` — this forwards the flag to the existing `/skill:phx-review --codex`
...
- `/skill:phx-document` for documentation generation
- `/skill:phx-learn-from-fix` to capture lessons learned
