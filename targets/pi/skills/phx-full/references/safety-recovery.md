...
```bash
/skill:phx-full --resume magic-link-auth
...
```bash
/skill:phx-work .claude/plans/magic-link-auth/plan.md --from P2-T3
```
...
```bash
/ralph-loop:ralph-loop "/skill:phx-full {feature}" --completion-promise "DONE" --max-iterations 50
```
...
```
/skill:phx-full {feature} --checkpoint-after plan
/skill:phx-full {feature} --checkpoint-after each-phase
```
