# Injectable Template for /skill:phx-init
...
<!-- Auto-generated by /skill:phx-init | {DATE} | Phoenix {PHOENIX_VERSION}, Ecto {ECTO_VERSION}{OPTIONAL_STACK} -->

# Elixir/Phoenix Plugin — Mandatory Procedures

## SKILL EXECUTION ENFORCEMENT

These rules govern ALL `/skill:phx-*` command execution. Violations invalidate the session output.
...
| Score | Action |
|-------|--------|
| ≤ 2 | Proceed directly, or offer `/skill:phx-quick` |
| 3-6 | Ask 1-2 questions, then `/skill:phx-plan` |
| 7-10 | Ask 2-4 questions, recommend `/skill:phx-plan --depth deep` |
| > 10 | Strongly recommend `/skill:phx-full` |
| Security ≥ 4 | MANDATORY: minimum `/skill:phx-plan` + spawn security-analyzer |
...
| Trigger | Agent | When |
|---------|-------|------|
| `/skill:phx-plan` invoked | phoenix-patterns-analyst | ALWAYS |
| `/skill:phx-plan` invoked | hex-library-researcher | ALWAYS |
...
| After | Offer |
|-------|-------|
| Bug fix | "Capture as lesson with /skill:phx-learn-from-fix?" |
| Feature complete | "Quality check with /skill:phx-review?" |
...
| Complex feature | `/skill:phx-plan` → `/skill:phx-work` → `/skill:phx-review` |
| Debug bug | `/skill:phx-investigate` |
| Review code | `/skill:phx-review` |
| Project health | `/skill:phx-audit` |
