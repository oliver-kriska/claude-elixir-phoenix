...
| Request | Path |
|---------|------|
| Bug, unexpected behavior, stack trace | `/skill:phx-investigate` |
| Small change (<50 lines), config, CSS tweak | `/skill:phx-quick` |
| Feature spanning 2+ contexts, or a new domain concept | `/skill:phx-plan` → `/skill:phx-work` → `/skill:phx-review` |
| Auth, sessions, tokens, payments, billing | `/skill:phx-plan` plus the `security-analyzer` agent — security work does not skip planning |
| Scope still unclear | `/skill:phx-brainstorm` |
