...
Detailed templates for each section of the `/skill:phx-brief` interactive
...
fix or architectural decision, suggest `/skill:phx-compound` to capture it.}
```
...
- Suggest `/skill:phx-compound` only when there's genuinely novel
...
3. If the answer isn't in the artifacts, say so: "The plan doesn't
   ...
   it or run `/skill:phx-plan --existing` to research this aspect"
