...
Task creation and planning happen in `$elixir-phoenix:phx-plan` after the user decides.
...
Ready for: $elixir-phoenix:phx-learn-from-fix (capture lessons)
```
...
- `$elixir-phoenix:phx-plan` — Plan tests for these functions
- `$elixir-phoenix:phx-work` — Write tests directly
...
- `$elixir-phoenix:phx-plan .claude/plans/{slug}/reviews/{file}.md` — Replan fixes
  (best for complex or architectural issues)
- `$elixir-phoenix:phx-work` — Fix directly
