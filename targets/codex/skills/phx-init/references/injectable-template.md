...
| Request | Path |
|---------|------|
| Bug, unexpected behavior, stack trace | `$elixir-phoenix:phx-investigate` |
| Small change (<50 lines), config, CSS tweak | `$elixir-phoenix:phx-quick` |
| Feature spanning 2+ contexts, or a new domain concept | `$elixir-phoenix:phx-plan` → `$elixir-phoenix:phx-work` → `$elixir-phoenix:phx-review` |
| Auth, sessions, tokens, payments, billing | `$elixir-phoenix:phx-plan` plus the `security-analyzer` agent — security work does not skip planning |
| Scope still unclear | `$elixir-phoenix:phx-brainstorm` |
...
## Running a `$elixir-phoenix:phx-*` skill
