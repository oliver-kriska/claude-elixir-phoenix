# Injectable Template for $elixir-phoenix:phx-init
...
<!-- Auto-generated by $elixir-phoenix:phx-init | {DATE} | Phoenix {PHOENIX_VERSION}, Ecto {ECTO_VERSION}{OPTIONAL_STACK} -->

# Elixir/Phoenix Plugin — Mandatory Procedures

## SKILL EXECUTION ENFORCEMENT

These rules govern ALL `$elixir-phoenix:phx-*` command execution. Violations invalidate the session output.
...
| Score | Action |
|-------|--------|
| ≤ 2 | Proceed directly, or offer `$elixir-phoenix:phx-quick` |
| 3-6 | Ask 1-2 questions, then `$elixir-phoenix:phx-plan` |
| 7-10 | Ask 2-4 questions, recommend `$elixir-phoenix:phx-plan --depth deep` |
| > 10 | Strongly recommend `$elixir-phoenix:phx-full` |
| Security ≥ 4 | MANDATORY: minimum `$elixir-phoenix:phx-plan` + spawn security-analyzer |
...
| Trigger | Agent | When |
|---------|-------|------|
| `$elixir-phoenix:phx-plan` invoked | phoenix-patterns-analyst | ALWAYS |
| `$elixir-phoenix:phx-plan` invoked | hex-library-researcher | ALWAYS |
...
| After | Offer |
|-------|-------|
| Bug fix | "Capture as lesson with $elixir-phoenix:phx-learn-from-fix?" |
| Feature complete | "Quality check with $elixir-phoenix:phx-review?" |
...
| Complex feature | `$elixir-phoenix:phx-plan` → `$elixir-phoenix:phx-work` → `$elixir-phoenix:phx-review` |
| Debug bug | `$elixir-phoenix:phx-investigate` |
| Review code | `$elixir-phoenix:phx-review` |
| Project health | `$elixir-phoenix:phx-audit` |
