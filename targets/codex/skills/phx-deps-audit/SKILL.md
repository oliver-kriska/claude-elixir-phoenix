---
name: phx-deps-audit
description: Audit Hex deps for supply-chain security risk — bidi chars; Use after
  mix deps.update, when checking if a package…
---
...
|------|---------|-----------|-----------|
| **B** (default) | `$elixir-phoenix:phx-deps-audit` | `git show HEAD:mix.lock` | working `mix.lock` |
| **C** (PR) | `$elixir-phoenix:phx-deps-audit --base main` | `git show <ref>:mix.lock` | working `mix.lock` |
| **A** (preview) | `$elixir-phoenix:phx-deps-audit --preview httpoison` | locked version | Hex API latest |
...
Use `$elixir-phoenix:phx-deps-vet <pkg> <version>` (separate skill) to add entries.
...
- **Prompt** for `$elixir-phoenix:phx-compound` after BLOCK findings — corpus self-feeds.
