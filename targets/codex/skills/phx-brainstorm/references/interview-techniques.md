...
Detailed methodology for the adaptive interview phase of `$elixir-phoenix:phx-brainstorm`.
...
- **Summary section** is what `$elixir-phoenix:phx-plan` reads first — must be self-contained
- **Coverage Details** replace `$elixir-phoenix:phx-plan`'s clarification questions
- **Codebase Context** replaces the patterns-analyst research in `$elixir-phoenix:phx-plan`
- **Transcript** at the bottom for audit trail — not consumed by `$elixir-phoenix:phx-plan`
