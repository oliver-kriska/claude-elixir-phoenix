...
```bash
$elixir-phoenix:phx-full --resume magic-link-auth
...
```bash
$elixir-phoenix:phx-work .claude/plans/magic-link-auth/plan.md --from P2-T3
```
...
```
$elixir-phoenix:phx-full {feature} --checkpoint-after plan
$elixir-phoenix:phx-full {feature} --checkpoint-after each-phase
```
