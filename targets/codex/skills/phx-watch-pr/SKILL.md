---
name: phx-watch-pr
description: Watch an Elixir/Phoenix PR for new review comments (bot + human); Use
  after opening a PR or pushing, while waiting on…
---
...
```
$elixir-phoenix:phx-watch-pr 42                 # watch reviews + comments + checks
$elixir-phoenix:phx-watch-pr 42 --checks-only   # CI only (delegates to gh pr checks --watch)
$elixir-phoenix:phx-watch-pr 42 --fix           # on actionable review, draft fixes too
$elixir-phoenix:phx-watch-pr 42 --codex         # + request Codex cloud review, loop until clean
$elixir-phoenix:phx-watch-pr 42 --codex --codex-rounds 2   # cap re-review rounds (default 3)
```
...
4. **NEVER auto-post replies or auto-push** — hand off to `$elixir-phoenix:phx-pr-review`
...
`$elixir-phoenix:phx-review --codex` / `$elixir-phoenix:phx-codex-loop` run are DIFFERENT mechanisms from
...
report the conclusion; on failure, offer `$elixir-phoenix:phx-investigate` with the
...
| Event | Action |
|-------|--------|
| `review` / `comment` (actionable) | Summarize the delta; with `--fix` draft fixes + `mix compile && mix test`; route reply drafting to `$elixir-phoenix:phx-pr-review {n}` |
| `check` conclusion `failure` | Offer `$elixir-phoenix:phx-investigate` on the failing job |
| `codex_ack` | Note "codex is reviewing (~15–20 min on large PRs)"; keep waiting |
| `codex_review` | Stop the watcher. Run `$elixir-phoenix:phx-pr-review {n} --bots-only` (fix → reply → resolve; user approves and pushes). If rounds < `--codex-rounds`: post `@codex review` again, restart watcher with the new trigger id, round+1. Else: report remaining findings, stop |
...
A `codex_review` whose `$elixir-phoenix:phx-pr-review --bots-only` fetch finds zero
...
```text
push / open PR → $elixir-phoenix:phx-watch-pr {n} ──(new review)──► $elixir-phoenix:phx-pr-review {n}
                              ├──────(CI fail)─────► $elixir-phoenix:phx-investigate
                              ├──(--codex: codex_review)─► $elixir-phoenix:phx-pr-review --bots-only → push → re-request → watch (≤3 rounds)
